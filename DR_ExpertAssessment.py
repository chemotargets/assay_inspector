...
def __checkEndpointDistribution(self, endpoint_distribution_results, meandiff_thresh, alpha=0.01):
    ...
    diff_distribution_df = diff_distribution_df.sort_values(by='meandiff', ascending=False).reset_index()
...
def __writeToTXT(self, diff_distribution_df, diff_range_df, skewed_distribution_df, outliers_identification_df, 
    ...
    with open(self.directory+'/'+self.endpoint_name+'/expert_assessment.txt','w') as f:
        f.write('###########################################################################\n')
        ...
        if self.task == config["TASK_REGRESSION"]:
            ...
            if diff_distribution_df.shape[0] > 0:
                f.write(f'Data sources with a significantly different {self.endpoint_name} distribution:\n')
                ...
                f.write(f'No data source shows a significantly different {self.endpoint_name} distribution compared to others.\n')
            ...
            if diff_range_df.shape[0] > 0:
                ...
                f.write(f'No data source shows an inconsistent {self.endpoint_name} value range (> 1 order of magnitude).\n')
            ...
            if skewed_distribution_df.shape[0] > 0:                
                f.write(f'Data sources with a skewed {self.endpoint_name} distribution:\n')
                for i,row in skewed_distribution_df.iterrows():
                    ...
                    if i+1 == skewed_distribution_df.shape[0]:
                        f.write(f'\t***Figure reference: {self.endpoint_name} Distribution Across Data Sources (Overlaid Histogram) [Fig 3.1].\n')  
            else:
                f.write(f'No data source shows a skewed {self.endpoint_name} distribution.\n')
            ...
            if outliers_identification_df.shape[0] > 0:                
                ...
                for i,row in outliers_identification_df.iterrows():
                    ...
                    if i+1 == outliers_identification_df.shape[0]:
                        f.write(f'\t***Figure reference: Visualization of {self.endpoint_name} Outliers [Fig 1].\n')  
            else:
        ...
        if conflicting_sources_df.shape[0] > 0:
            f.write(f'Data source pairs with conflicting {self.endpoint_name} value annotations for shared molecules\n(>50% molecules in common + endpoint differences > 10%):\n')
            ...
            f.write(f'No data source pair has conflicting {self.endpoint_name} value annotations for shared molecules.\n')
...
###
def __generateExpertAssessment(self, data_df, skewness_results, outliers_results, oor_results, 
