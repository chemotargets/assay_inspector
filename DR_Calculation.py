...
### 
def __getNmols_perDataset(self, data_df):
...
### 
def __calculatePropRefMols(self, data_df, ref_set):
...
### 
def __calculatePropRefMols_perDataset(self, data_df, ref_set):
...
###
def __calculateEndpointStatistics(self, data_df):
...
### 
def __calculateEndpointStatistics_perDataset(self, data_df):
...
### 
def __calculateSkewness(self, data_df):
...
### 
def __calculateSkewness_perDataset(self, data_df):
...
### 
def __calculateKurtosis(self, data_df):
...
### 
def __calculateKurtosis_perDataset(self, data_df):
...
### 
def __identifyOutliers(self, data_df):
...
### 
def __identifyOutliers_perDataset(self, data_df):
...
### 
def __identifyOORDataPoints(self, data_df):
...
### 
def __identifyOORDataPoints_perDataset(self, data_df):
...
### 
def __compareEndpointDistribution_acrossDatasets(self, data_df):
...
### 
def __calculateDistanceMatrix(self, data_df):
    ...
    if self.feature_type == 'rdkit': 
        variances = np.var(data_df.loc[:, [f'{self.features}_{feature}' for feature in MoleculeInfo.AVAILABLE_FEATURES[self.features]]].to_numpy(), axis=0, ddof=1) # compute feature variances
        variances[variances == 0] = 1e-8 # replace zero variances with a small positive value to avoid division by zero
        distance_matrix = pairwise_distances(data_df.loc[:, [f'{self.features}_{feature}' for feature in MoleculeInfo.AVAILABLE_FEATURES[self.features]]].to_numpy(), metric='seuclidean', V=variances)

    elif self.feature_type == 'ecfp4': 
        indices = pairwise_distances(data_df.loc[:, [f'{self.features}_{feature}' for feature in MoleculeInfo.AVAILABLE_FEATURES[self.features]]].to_numpy(), metric='jaccard') 
        distance_matrix = 1 - indices

    return distance_matrix

### 
def __calculateFeatureSimilarity(self, distance_matrix, data_df):
...
### 
def __calculateFeatureSimilarity_Pairwise(self, distance_matrix, data_df):
...
###
def __calculateInterDatasetDiscrepancies(self, data_df):
...
### 
def __perfromPairwiseKSTest(self, data_df):
