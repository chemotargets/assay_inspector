...
def __getAxisRanges(self, proj, expand_factor=0.1):
    ...
    largest_range = np.max(proj[:, 0].max()-proj[:, 0].min(), proj[:, 1].max()-proj[:, 1].min())
    
    # Calculate midpoints, expand the largest range, and set both axes to have the same range
    x_mid = (proj[:, 0].max() + proj[:, 0].min()) /2
    y_mid = (proj[:, 1].max() + proj[:, 1].min()) /2
...
###
def __VisualizeOutliers(self, data_df, outliers_set):
    ...
    # Save the figure
    pio.write_image(fig, os.path.join(self.directory, self.endpoint_name, '1_outlier_visualization.svg'), format='svg')

###
def __plotEndpointDistribution(self, data_df, outliers_set):
    ...
    if self.task == config["TASK_CLASSIFICATION"]:
        ...
        fig = go.Figure(data=[go.Bar(x=value_counts.index, y=value_counts.values,
                                     ...
                                     marker_color=self.hex_colors[:len(value_counts.index)])])
        fig.update_layout(
            title={'text':f'[2] Distribution of {self.endpoint_name} Classes', 'x':0.5, 'xanchor':'center', 'font':{'size':24}},
            xaxis_title={'text':f'{self.endpoint_name} classes', 'font':{'size':16}},
    ...
    # Histogram and Violin plot for regression endpoints
        ...
        fig.update_layout(
            title={'text':f'[2.2] Distribution of {self.endpoint_name} Values', 'x':0.5, 'xanchor':'center', 'font':{'size':24}},
            yaxis_title={'text':f'{self.endpoint_name}', 'font':{'size':16}},
        ...
        pio.write_image(fig, os.path.join(self.directory, self.endpoint_name, '2_2_endpoint_distribution_visualization_violinplot.svg'), format='svg')
    ...
    if self.task == config["TASK_REGRESSION"]:
        ...
        fig.update_layout(
            title={'text':f'[2.3] Distribution of {self.endpoint_name} Values', 'x':0.5, 'xanchor':'center', 'font':{'size':24}},
            yaxis_title={'text':f'{self.endpoint_name} value', 'font':{'size':16}},
        ...
        # Save the figure
        pio.write_image(fig, os.path.join(self.directory, self.endpoint_name, '2_3_endpoint_distribution_visualization_outliers.svg'), format='svg')

###
def __plotEndpointDistributionComparison(self, data_df, outliers_dict):
    ...
    if self.task == config["TASK_CLASSIFICATION"]:
        ...
        for i, ref in enumerate(self.sources_list):
            ...
            for j in range(len(percentages)):
                bars_percentage.append(go.Bar(name=str(percentages.index[j]), x=[ref], y=[percentages.get(j, 0)], 
                                              marker_color=self.hex_colors[j], opacity=0.7, showlegend=False))    
            # Class-Counts Bar Chart
            for k in range(len(counts)):
                bars_counts.append(go.Bar(name=str(counts.index[k]), x=[ref], y=[counts.get(k, 0)], 
                                          marker_color=self.hex_colors[k], opacity=0.7, legendgroup=str(counts.index[j]), showlegend=(i==0),
        ...
        fig.update_layout(
            barmode='stack',
            title={'text': f'[3] Distribution of {self.endpoint_name} Classes Across Data Sources', 'x': 0.5, 'xanchor': 'center', 'font': {'size': 24}},
            ...
            legend_title={'text':f'{self.endpoint_name} classes', 'font':{'size':16}},
    ...
    # Overlaid Histogram and Multiple Violin plots for regression endpoints
        ...
        fig.update_layout(
            title={'text':f'[3.2] Distribution of {self.endpoint_name} Values Across Data Sources', 'x':0.5, 'xanchor':'center', 'font':{'size':24}},
            xaxis3_title={'text': 'Data Sources', 'font': {'size': 16}},  
            yaxis=dict(title={'text': 'Nº of Mols', 'font': {'size': 10}}),  
            ...
            yaxis3=dict(title={'text': f'{self.endpoint_name} value', 'font': {'size': 16}}, range=yaxis_range), 
        ...
        pio.write_image(fig, os.path.join(self.directory, self.endpoint_name, '3_2_endpoint_distribution_comparative_visualization_violinplots.svg'), format='svg')
    ...
    if self.task == config["TASK_REGRESSION"]:
        ...
        violins.append(go.Violin(y=[None], name='Outliers included', legendgroup='Outliers included', line_color=self.hex_colors[0], showlegend=True))
        violins.append(go.Violin(y=[None], name='Outliers excluded', legendgroup='Outliers excluded', line_color=self.hex_colors[1], showlegend=True))
        ...
        fig.update_layout(
            title={'text':f'[3.3] Distribution of {self.endpoint_name} Values Across Data Sources', 'x':0.5, 'xanchor':'center', 'font':{'size':24}},
            yaxis_title={'text':f'{self.endpoint_name} value', 'font':{'size':16}},
            height=500, width=1100,
            margin=dict(l=50, r=50, t=50, b=50),
            showlegend=True,
            xaxis=dict(range=[1.6, len(self.sources_list)+1.4]),
            yaxis=dict(range=yaxis_range)
        )

        # Save the figure
        pio.write_image(fig, os.path.join(self.directory, self.endpoint_name, '3_3_endpoint_distribution_comparative_visualization_outliers.svg'), format='svg')

###
def __plotEndpointDistributionComparison_RefVsNonRefMols(self, data_df, ref_set):
    ...
    if self.task == config["TASK_CLASSIFICATION"]:
            ...
            for j in range(len(percentages_ref_mols)):
                if j == 0:
                    bars_percentages_refmols.append(go.Bar(name=f'{str(percentages_ref_mols.index[j])} (Reference molecules)', x=[ref], y=[percentages_ref_mols.get(j, 0)], 
                                                           marker_color=self.hex_colors[j], showlegend=(i==0), offsetgroup='ref', opacity=0.7)) 
                    bars_percentages_refmols.append(go.Bar(name=f'{str(percentages_nonref_mols.index[j])} (Non-Reference molecules)', x=[ref], y=[percentages_nonref_mols.get(j, 0)], 
                                                           marker_color=self.hex_colors[j+2], showlegend=(i==0), offsetgroup='nonref', opacity=0.7)) 
                else:
                    bars_percentages_refmols.append(go.Bar(name=f'{str(percentages_ref_mols.index[j])} (Reference molecules)', x=[ref], y=[percentages_ref_mols.get(j, 0)], 
                                                           marker_color=self.hex_colors[j], showlegend=(i==0), base=[percentages_ref_mols.get(j-1, 0)], offsetgroup='ref', opacity=0.7)) 
                    bars_percentages_refmols.append(go.Bar(name=f'{str(percentages_nonref_mols.index[j])} (Non-Reference molecules)', x=[ref], y=[percentages_nonref_mols.get(j, 0)], 
                                                           marker_color=self.hex_colors[j+2], showlegend=(i==0), base=[percentages_nonref_mols.get(j-1, 0)], offsetgroup='nonref', opacity=0.7)) 
            ...
            for k in range(len(counts_ref_mols)):
                if k == 0:
                    bars_counts_refmols.append(go.Bar(name=f'{str(counts_ref_mols.index[k])} (Reference molecules)', x=[ref], y=[counts_ref_mols.get(k, 0)], 
                                                      marker_color=self.hex_colors[k], showlegend=False, offsetgroup='ref', opacity=0.7)) 
                    bars_counts_refmols.append(go.Bar(name=f'{str(counts_nonref_mols.index[k])} (Non-Reference molecules)', x=[ref], y=[counts_nonref_mols.get(k, 0)], 
                                                      marker_color=self.hex_colors[k+2], showlegend=False, offsetgroup='nonref', opacity=0.7)) 
                else:
                    bars_counts_refmols.append(go.Bar(name=f'{str(counts_ref_mols.index[k])} (Reference molecules)', x=[ref], y=[counts_ref_mols.get(k, 0)], 
                                                      marker_color=self.hex_colors[k], showlegend=False, base=[counts_ref_mols.get(k-1, 0)], offsetgroup='ref', opacity=0.7,
                                                      text=f"{prop_ref_mols}", texttemplate='%{text:.2f}%', textposition='outside')) 
                    bars_counts_refmols.append(go.Bar(name=f'{str(counts_nonref_mols.index[k])} (Non-Reference molecules)', x=[ref], y=[counts_nonref_mols.get(k, 0)], 
                                                      marker_color=self.hex_colors[k+2], showlegend=False, base=[counts_nonref_mols.get(k-1, 0)], offsetgroup='nonref', opacity=0.7,
        ...
        fig.update_layout(
            barmode='group',
            title={'text': f'[ADD.] Comparison of {self.endpoint_name} Classes for Reference vs. Non-Reference Molecules Across Data Sources', 'x': 0.5, 'xanchor': 'center', 'font': {'size': 20}},
    ...
    # Mutliple Split Violin plots for regression endpoints
        ...
        # Add dummy traces for legend entries
        violins.append(go.Violin(y=[None], name='Reference molecules', legendgroup='Reference molecules', line_color=self.hex_colors[1], showlegend=True))
        violins.append(go.Violin(y=[None], name='Non-reference molecules', legendgroup='Non-reference molecules', line_color=self.hex_colors[2], showlegend=True))
        ...
        for ref in self.sources_list:
            ...
            bars_refmols.append(go.Bar(name='Reference molecules', x=[ref], y=[prop_ref_mols], marker_color=self.hex_colors[1], showlegend=False, opacity=0.7))
        ...
        fig.update_layout(
            barmode='stack',
            title={'text':f'[EXTRA] Comparison of {self.endpoint_name} Values for Reference vs. Non-Reference Molecules Across Data Sources', 'x':0.5, 'xanchor':'center', 'font':{'size':20}},
            xaxis3_title={'text': 'Data Sources', 'font': {'size': 16}},  
            yaxis=dict(title={'text': 'Nº of Mols', 'font': {'size': 10}}),  
            yaxis2=dict(title={'text': 'Proportion (%)', 'font': {'size': 10}}, range=[0, 100], minor=dict(nticks=4, ticklen=5, tickcolor="black", showgrid=True)), 
            yaxis3=dict(title={'text': f'{self.endpoint_name} value', 'font': {'size': 16}}, range=yaxis_range), 
    ...
    # Save the figure
    pio.write_image(fig, os.path.join(self.directory, self.endpoint_name, 'EXTRA_endpoint_distribution_comparative_visualization_refmols.svg'), format='svg')

###
def __plotFeatureSpace_coloredbyEndpoint(self, proj, data_df):
    ...
    if self.task == config["TASK_CLASSIFICATION"]:
        ...
        fig.update_layout(
            ...
            xaxis_title={'text':f'{self.dimensionality_technique}1', 'font':{'size':16}},
            yaxis_title={'text':f'{self.dimensionality_technique}2', 'font':{'size':16}},
            legend={'font': {'size': 16}},
    ...
    # Save the figure
    fig.write_image(os.path.join(self.directory, self.endpoint_name, '4_UMAP_projection_coloredby_endpoint.svg'), format='svg')


###
def __plotFeatureSpace_coloredbyDataset(self, proj, data_df):
    """
    ...
    # Save the figure
    fig.write_image(os.path.join(self.directory, self.endpoint_name, '5_UMAP_projection_coloredby_dataset.svg'), format='svg')

###
def __plotFeatureSpace_KDEplot(self, proj, data_df):
    ...
    if len(self.hex_colors) < len(self.sources_list):
        palette = [color for color, _ in zip(cycle(self.hex_colors), range(len(self.sources_list)))]
    else:
        palette = self.hex_colors[:len(self.sources_list)]
    ...
    plt.ylabel(f'{self.dimensionality_technique}2', fontsize=14)
    ...
    # Save the figure
    plt.savefig(os.path.join(self.directory, self.endpoint_name, '6_UMAP_projection_KDEplot.svg'), format='svg')
    plt.close()

###
def __plotFeatureSpace_Hexbin(self, proj):
    ...
    fig = go.Figure(go.Histogram2d(x=proj[:, 0], y=proj[:, 1], histnorm='density', 
                                   autobinx=False, xbins=dict(start=self.x_range[0], end=self.x_range[1], size=(self.x_range[1] - self.x_range[0])/100),
                                   autobiny=False, ybins=dict(start=self.y_range[0], end=self.y_range[1], size=(self.y_range[1] - self.y_range[0])/100),
    ...
    # Save the figure
    pio.write_image(fig, os.path.join(self.directory, self.endpoint_name, '7_UMAP_projection_Hexbin.svg'), format='svg')

###
def __plotDatasetsIntersection(self, data_df):
    ...
    if len(mols_per_source) == 2: 
        ...
        set1 = set(mols_per_source[self.sources_list[0]])
        set2 = set(mols_per_source[self.sources_list[1]])
        # Plot the Venn diagram
        plt.figure(figsize=(8, 8))
        venn2([set1, set2], (self.sources_list[0], self.sources_list[1]), set_colors=(self.hex_colors[0], self.hex_colors[1]))
    ...
    # 3 DATASETS
        ...
        set1 = set(mols_per_source[self.sources_list[0]])
        set2 = set(mols_per_source[self.sources_list[1]])
        set3 = set(mols_per_source[self.sources_list[2]])
        # Plot the Venn diagram
        plt.figure(figsize=(8, 8))
        venn3([set1, set2, set3], (self.sources_list[0], self.sources_list[1], self.sources_list[2]), set_colors=(self.hex_colors[0], self.hex_colors[1], self.hex_colors[2]))
        plt.title('[8] Venn Diagram', fontsize=24)
        plt.tight_layout()
    ...
    # MORE THAN 3 DATASETS
        ...
        if len(self.sources_list) == 4: # 4 data sources
            upset = UpSet(upset_data, subset_size='count', sort_by='cardinality', show_counts="{:,}", facecolor=self.hex_colors[0], element_size=40) 
        else:
            try: 
    ...
    # Save the image
    plt.savefig(os.path.join(self.directory, self.endpoint_name, '8_intersection_across_datasets.svg'), format='svg')
    plt.close()

###
def __plotComparativeHeatmaps(self, info_df):
    ...
    num_common_mols_df = num_common_mols_df.loc[self.sources_list[::-1], self.sources_list] # reorder heatmap matrix
    percent_common_mols_df = info_df.pivot(index='source_1', columns='source_2', values='percent_common_mols')
    percent_common_mols_df = percent_common_mols_df.loc[self.sources_list[::-1], self.sources_list] # reorder heatmap matrix
    ...
    # Create the figure
    fig = make_subplots(rows=1, cols=2, subplot_titles=('Common Molecules Counts', f'{self.endpoint_name} Value Difference'), shared_yaxes=True)
    ...
    # Save the figure
    pio.write_image(fig, os.path.join(self.directory, self.endpoint_name, '9_comparative_heatmaps.svg'), format='svg')

###
def __plotSimilarityDistribution(self, distance_matrix):
    ...
    # Create the figure
    fig = go.Figure(data=[go.Histogram(x=random_subset, marker_color=self.hex_colors[0])])
    fig.update_layout(
    ...
    # Save the figure
    pio.write_image(fig, os.path.join(self.directory, self.endpoint_name, '10_intermolecular_distances_visualization.svg'), format='svg')

###
def __plotFeatureSimilarityHeatmap(self, similarity_results):
    ...
    similarity_table = similarity_results.pivot(index='ref1', columns='ref2', values='average_between_similarity') 
    similarity_table = similarity_table.loc[self.sources_list, self.sources_list] # reorder heatmap matrix
    ...
    # Save the figure
    pio.write_image(fig, os.path.join(self.directory, self.endpoint_name, '11_interdataset_distances_heatmap.svg'), format='svg')

def __plotPairwiseKSTestHeatmap(self, ks_results):
    ...
    # Save the figure
    pio.write_image(fig, os.path.join(self.directory, self.endpoint_name, '12_kstest_heatmap.svg'), format='svg')
